...
-Imprima los nombres completos (nombre y apellidos) de las personas que están en un rango de edades dado por el usuario.


Desarrollo: 
```
import json #Llamando al yeison 0_0



with open(r"D:\Users\ASUS\Desktop\jeizon.txt","r") as archivo_json:

    a = json.load(archivo_json) #Utilizo load para llamar al archivo json guardado previamente
    llaves = (a.keys())
    n = (input("Ingrese que deporte: ")) #Ahora con cada n que el usuario ingrese buscaré en cada key del yeison xd

    for i in llaves:

        if n.lower() in a[i]["deportes"].lower(): #Utilizo.lower para hacer el código más robusto en caso de que el usuario ingres alguna letra en mayúscula
            print(a[i]["nombres"],a[i]["apellidos"]) #Puesto que cada key tiene otro json vomo value, por cada key que tenga como value ese key, buscaré los deportes

```

Desarrollo punto 4

El siguiente código contiene un JSON con el pronóstico detallado del clima para 8 días:
import json
...
```
Revisa los campos: 'alertAlertas', 'alertPrecip', 'alertTmpMax', 'alertTmpMin', 'alertVelViento'. Para cada uno identifique si se presentan alertas ({0: x} indica que el día 0 habrá un fenómeno de la alerta en cuestión, {1:"-"} indica que no habrá ningún fenómeno climático). En caso que se presente una alerta obtenga la fecha del campo 'dt' ( aquí pueden revisar como se convierte de UTC a fecha), así como los parámetros relevantes del evento (eg si es un fenomeno de lluvias, busqye el nivel de lluvia, si es vientos, la velocidad del viento). Al final deberá imprimir las fechas de alerta, el tipo de alerta y las variables asociadas.
...
# Codigo tomado de stackoverflow de como pasar el numero a fecha
from datetime import datetime
ts = int('1284101485')

#print(datetime.utcfromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S'))
#'alertPrecip', 'alertTmpMax', 'alertTmpMin', 'alertVelViento'

dt = data["dt"]

listaa_ = [data["alertAlertas"],data['alertPrecip'], data['alertTmpMax'],data['alertTmpMin'], data['alertVelViento']]
for i in listaa_:
    for j in i:
    
        if i[j].lower() == 'X'.lower():
            print(f"El dia {j} Habrá fenomeno climático, los datos son: ")
            print("Fecha y hora:",datetime.utcfromtimestamp(dt[j]).strftime('%Y-%m-%d , %H:%M:%S'))

        elif i[j] == '-':
            print(f"El día {j} no habrán fenomenos climáticos")
```
